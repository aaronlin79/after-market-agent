...
def cluster_articles(
    ...
    run = start_pipeline_run(
        ...
        run_type="clustering",
        provider_used="local",
        metrics_json={"similarity_threshold": similarity_threshold},
    )
    ...
    try:
        ...
        if article_count == 0:
            ...
            stats = ClusterStats(article_count=0, cluster_count=0, representative_count=0).to_dict()
        ...
        fail_pipeline_run(db, run, error=exc, metrics_json={"similarity_threshold": similarity_threshold}, provider_used="local")
